...
```bash
curl -X PUT -T exasol-kinesis-connector-extension-1.1.1.jar \
  http://w:write-password@exasol.datanode.domain.com:2580/kinesis/exasol-kinesis-connector-extension-1.1.1.jar
```
...
EMITS (KINESIS_SHARD_ID VARCHAR(130), SHARD_SEQUENCE_NUMBER VARCHAR(2000)) AS
  ...
  %jar /buckets/bfsdefault/kinesis/exasol-kinesis-connector-extension-1.1.1.jar;
/
;
